Feature: Rules
  ...
  Scenario Outline: Test that the rule user matching works (User {<attributeKey>: <attributeValue>} with operator: <operator>, target: <target>, values: <values>, result: <result>)
  ...
  Examples:
    | attributeKey    | attributeValue    | operator | target   | values            | result |
