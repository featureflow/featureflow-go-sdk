Feature: Hashing Algorithm
  Scenario Outline: Testing that the user id "<userId>", salt "<salt>" and feature "<feature>" returns the result "<result>"
    Given the salt is "<salt>", the feature is "<feature>" and the user id is "<userId>"
  ...
  Examples:
    | salt  | feature   | userId        | result | hash            |
